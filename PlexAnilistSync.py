...
class PlexAnilistSync:
    def __init__(self, plex_anilist_matcher, config):
        try:
            self.anilist_helper = AnilistHelper(config["anilist_username"], config["anilist_token"])
            self.anilist_user = config["anilist_username"]
            self.plex_anime_helper = PlexAnimeHelper(
                config["plex_username"], config["plex_password"], config["plex_server_dict"].keys()
            )
            self.plex_anilist_matcher = plex_anilist_matcher
            self.plex_server_dict = config["plex_server_dict"]
            self.ok = True
    ...
    def sync(self):
        anilist_list = self.anilist_helper.get_user_list()
        if not anilist_list:
            LOGGER.warn("Could not get anilist")
            return
        LOGGER.info("Found %s anime entries in %s anilist", len(anilist_list), self.anilist_user)
        plex_watched_list = []
        for server in self.plex_server_dict.keys():
            for section in self.plex_server_dict[server]:
                plex_watched_list.extend(
                    self.plex_anime_helper.get_shows_with_watched_episodes(server, section)
                )
        LOGGER.info("Found %s anime with watched episodes", len(plex_watched_list))
        for plex_anime_entry in plex_watched_list:
            anilistId = self.plex_anilist_matcher.map_plex_guid_to_anilist_id(plex_anime_entry.guid)
            LOGGER.info("-----------------------------------------------------------------")
            if anilistId:
                ...
                if anilist_entry:
                    ...
                    if anilist_entry["status"] == "COMPLETED":
                        ...
                        continue
                    plex_watched_episodes = self.plex_anime_helper.get_watched_episodes(
                        plex_anime_entry
                    )
                    ...
                    if plex_watched_episodes > anilist_entry["progress"]:
                        if (
                            ...
                            and plex_watched_episodes >= anilist_entry["media"]["episodes"]
                            ...
                            self.anilist_helper.update_user_anilist_entry(
                                anilistId,
                                min(anilist_entry["media"]["episodes"], plex_watched_episodes),
                                "COMPLETED",
                            )
                        else:
                            if anilist_entry["status"] == "PLANNING":
                                ...
                                self.anilist_helper.update_user_anilist_entry(
                                    anilistId, plex_watched_episodes, anilist_entry["status"]
                                )
                    else:
                    ...
                    LOGGER.warning(
                        "Anime with id: %s not in list. Looking for it",
                        anilistId
                    )
                    anilist_media_entry = self.anilist_helper.get_by_id(anilistId)
                    if anilist_media_entry:
                        LOGGER.info("Found anime: %s", anilist_media_entry["title"]["romaji"])
                        plex_watched_episodes = self.plex_anime_helper.get_watched_episodes(
                            plex_anime_entry
                        )
                        if plex_watched_episodes == 0:
                        ...
                        if (
                            ...
                            and plex_watched_episodes >= anilist_media_entry["episodes"]
                            ...
                            self.anilist_helper.update_user_anilist_entry(
                                anilistId,
                                min(anilist_media_entry["episodes"], plex_watched_episodes),
                                "COMPLETED",
                            )
                        else:
                            LOGGER.info(
                ...
                LOGGER.warning(
                    "Could not match id from Plex guid with anilist id: %s", plex_anime_entry.guid
                )
